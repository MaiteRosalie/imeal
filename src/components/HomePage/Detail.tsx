...
export const Detail = ({ recipe: {  title, description, ingredients, steps, cuisine, nutrition, id } }: DetailProps) => {
  const { setRatingNote, ratings, activeTimers, addTimer, removeTimer } = useAppState();
  const [tmpRating, setTmpRating] = useState<number>(0);
  const [tmpNote, setTmpNote] = useState<string>("");
  const [timerCountdowns, setTimerCountdowns] = useState<Map<number, number>>(new Map());
  ...
  const handleTimer = (seconds: number, minutes: number, stepIndex: number) => {
    ...
    setTimerCountdowns(prev => new Map(prev).set(stepIndex, seconds));
    ...
    const countdown = setInterval(() => {
      setTimerCountdowns(prev => {
        ...
        const current = newMap.get(stepIndex) || 0;
        if (current <= 1) {
          newMap.delete(stepIndex);
          return newMap;
        }
        newMap.set(stepIndex, current - 1);
        return newMap;
      });
    }, 1000);
    ...
    const timer = setTimeout(() => {
      ...
      if (typeof window !== "undefined") {
        ...
        if ("Notification" in window && Notification.permission === "granted") {
          ...
          alert(`⏰ Timer finished! Step ${stepIndex + 1} is complete: ${steps[stepIndex].text}`);
        }
      }
    ...
    countdownRefs.current.set(stepIndex, countdown);
  };
    ...
    <div>
      ...
      <div>
        ...
        <ul className="list-disc pl-5 text-sm">
          {steps.map((s, idx) => {
            ...
            const isTimerActive = activeTimers.some(t => t.recipeId === id && t.stepIndex === idx);
      ...
      <div className="space-y-2">
        ...
        <form onSubmit={(e)=>{
          ...
          <input className="border rounded p-1 flex-1 ml-2" placeholder="Notes..." value={tmpNote} onChange={(e) => setTmpNote(e.target.value)} />
          <button
